...
//int main_fishEye_distored(int argc,char *argv[])
int main_fishEye_distored( string   nameOfImage )
{
	...
	**/
	...
	bool should_show = 1;  //parser.has("S") ? true : false;
	bool do_clip = 1;// parser.has("C") ? true : false;
	sEnlage = 2.0;// 3.0;// 2.0;// parser.get<double>("X");
	cv::String imagefile = cv::String(nameOfImage.c_str()  );// "sample.png";// parser.get<cv::String>("@image");
	cv::String resultfile = "sample_undistor.png";// parser.get<cv::String>("@result");
	FocalLengthX = 0.421427;
	FocalLengthY = 0.421427;
	ImageXCenter = 0.500484;
	ImageYCenter = 0.507854;
	RadialDistortParam1 = -0.020758;
	RadialDistortParam2 = -0.003717;
